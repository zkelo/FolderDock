...
- **Контекстное меню Проводника** (ПКМ по элементу): настоящее системное меню шелла — «Открыть с помощью», «Отправить», Копировать/Вырезать/Удалить/Переименовать, сторонние расширения (7-Zip, TortoiseGit, …) — ровно как в Проводнике. В окно менеджера встроен **настоящий вид Проводника** (`IExplorerBrowser`) с папкой ярлыков: ПКМ по ярлыку → **«Закрепить на панели задач»**, переименование, удаление, перетаскивание — без перехода в Проводник.
...
3. ПКМ по ярлыку во встроенном виде Проводника → **«Закрепить на панели задач»** (настоящее меню Проводника — закрепление, переименование, удаление, свойства без выхода из менеджера).
...
| `ManagerWindow.xaml(.cs)` | Менеджер: добавление папок, встроенный вид Проводника с ярлыками |
| `ExplorerBrowserHost.cs` | Хост IExplorerBrowser: нативный вид Проводника внутри менеджера |
